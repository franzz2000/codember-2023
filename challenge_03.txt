...
const cuenta = function(string, letra) {
  
  let contador = 0
  
  for (i=0; i < string.length; i++) {
    if (string[i] === letra) contador ++ 
  }
  return contador
}
...
let cuenta_incorrectas = 0

lineas.forEach((linea, indice) => {
    const datos = linea.split(/: |-|\s|: /g)
    const cuentas = cuenta(datos[3], datos[2])
    if(cuentas< datos[0] | cuentas> datos[1]) {
      cuenta_incorrectas++
    }
